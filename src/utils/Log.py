...
class  studentlog():
    ...
    def insert_time(self):
        ...
        self.database.c.execute(
            "INSERT INTO student_log_time (id_number,gender,log_time,type ) \
      VALUES (?, ?,?,?)",
            (self.item["id_number"], self.item["gender"],datetime.datetime.now().strftime("%Y-%m-%d-%H-%M"),1))
    ...
    def insert_cout(self):
        if self.item["count"] == None:
            cout = 1
            self.database.c.execute(
            "UPDATE student SET count = {0} WHERE id_number = {1}".format(cout,self.item["id_number"]),)
            self.database.conn.commit()
           
        else:
            cout = self.item["count"] + 1
            self.database.c.execute(
            "UPDATE student SET count = {0} WHERE id_number = {1}".format(cout,self.item["id_number"]),)
            self.database.conn.commit()
