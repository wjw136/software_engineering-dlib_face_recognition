...
class UpdatePwd(QDialog):
    ...
    def btn1_update_pwd(self):
        ...
        if new_pwd != new_pwd_2:
            ...
            database = Database()
            item = database.c.execute("select password ,salt from student where id_number = {0}".format(self.id_number)).fetchone()
            ...
            if old_pass_word == item["password"]:
                ...
                database.c.execute("update student set password = ? where id_number = {0}".format(self.id_number),(new_pass_word,))
